...
def listar_desfechos():
    ...
    with db_conn() as conn:
        ...
        # "Peça chegou" (pedido de 2026-09-18, CORRIGIDO em 2026-09-21):
        # reaproveita pecas_chegada — a MESMA tabela e a MESMA rota
        # (POST /pedidos/chegada) que "Peças > Pedidos com comprovante" já
        # usa desde 2026-09-03 pra essa pergunta. Uma primeira versão desta
        # função criou colunas e endpoints próprios (chegou_em/chegou_por em
        # servico_desfecho e pedido_peca_os) sem perceber que esse mecanismo
        # já existia — resultado: duas fontes de verdade que não se falavam
        # (marcar "chegou" aqui não refletia lá, e vice-versa). Consolidado
        # numa fonte só.
        if linhas:
            chaves_peca = [l["chave"] for l in linhas if l["desfecho"] == "precisa_peca"]
            if chaves_peca:
                marcadores = ",".join("?" * len(chaves_peca))
                chegadas = fetch_all(conn, sql(
                    f"SELECT chave, chegou_em, ordem_servico_id FROM pecas_chegada "
                    f"WHERE chave IN ({marcadores})"), tuple(chaves_peca))
                chegou_por_chave = {c["chave"]: c for c in chegadas}
                for l in linhas:
                    info = chegou_por_chave.get(l["chave"])
